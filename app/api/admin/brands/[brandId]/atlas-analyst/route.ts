...
import { listAtlasContextEntries } from "@/lib/brandops/ai/context-store";
import {
...
export async function POST(
  ...
  try {
    ...
    const response = await runAtlasAnalyst(request, {
      brandId,
      question,
...
export async function PATCH(
  ...
  try {
    ...
    if (!body.runId?.trim()) {
      throw new Error("Informe a execução do Atlas Analyst que receberá o feedback.");
    }

    if (body.vote !== "helpful" && body.vote !== "not_helpful") {
      throw new Error("Feedback inválido para o Atlas Analyst.");
    }
