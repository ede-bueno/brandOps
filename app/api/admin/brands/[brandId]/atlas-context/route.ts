...
} from "@/lib/brandops/ai/context-store";
import {
import type { AtlasContextEntry, AtlasContextEntryPayload } from "@/lib/brandops/ai/types";
  ...
  try {
    ...
    return NextResponse.json({
      summary: entries.length
        ? `${entries.length} contexto(s) curado(s) recente(s) ajudam o Atlas a lembrar campanhas, lançamentos, ajustes e incidentes da marca.`
        : null,
      updatedAt: latestEvent,
      items: entries.map((entry: AtlasContextEntry) => ({
        id: entry.id,
        kind:
          entry.entryType === "campaign"
            ? "campaign"
            : entry.entryType === "promotion"
              ? "promotion"
              : entry.entryType === "launch"
                ? "launch"
...
export async function POST(
  ...
  try {
    ...
    const entry = await createAtlasContextEntry(
      context.supabase,
      context.user.id,
      brandId,
      {
