  ...
  return {
    model:
      typeof source.model === "string" && source.model.trim()
        ...
        : DEFAULT_MODEL,
    credentialSource:
      source.credentialSource === "brand_key" ? ("brand_key" as const) : ("platform_key" as const),
...
export async function resolveAtlasAnalystGeminiAccess(brandId: string) {
  ...
  if (integration.settings.credentialSource === "brand_key") {
    let brandSecret;
    try {
    ...
    if (!brandSecret?.secret) {
      throw new Error("A marca está configurada para usar chave própria do Gemini, mas nenhuma chave foi salva no painel.");
    }
  ...
  if (!platformKey) {
    throw new Error("A marca está configurada para usar a chave da plataforma, mas GEMINI_API_KEY não está configurada no servidor.");
  }

  return {
    apiKey: platformKey,
    model: integration.settings.model || DEFAULT_MODEL,
    source: "platform_key" as const,
    secretHint: null,
  };
}
