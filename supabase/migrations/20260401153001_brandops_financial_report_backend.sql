...
DROP VIEW IF EXISTS public.v_financial_monthly_canonical;

CREATE OR REPLACE VIEW public.v_financial_monthly_canonical AS
WITH monthly_metrics AS (
  SELECT
    dm.brand_id,
    to_char((dm.sale_day)::timestamp with time zone, 'YYYY-MM'::text) AS yearmonth,
    SUM(COALESCE(dm.gross_revenue, 0)) AS gross_revenue,
    SUM(COALESCE(dm.discount_value, 0)) AS discount_value,
    SUM(COALESCE(dm.net_revenue, 0)) AS net_revenue,
    SUM(COALESCE(dm.commission_total, 0)) AS commission_total,
    SUM(COALESCE(dm.cmv_total, 0)) AS cmv_total,
    SUM(COALESCE(dm.gross_margin, 0)) AS gross_margin,
    SUM(COALESCE(dm.adcost, 0)) AS adcost,
    SUM(COALESCE(dm.contribution_margin, 0)) AS contribution_after_media,
    SUM(COALESCE(dm.items_sold, 0)) AS items_sold,
    SUM(COALESCE(dm.qty_real, 0)) AS qty_real,
    SUM(COALESCE(dm.order_count, 0)) AS order_count
  FROM public.v_daily_metrics dm
  GROUP BY dm.brand_id, to_char((dm.sale_day)::timestamp with time zone, 'YYYY-MM'::text)
),
monthly_expenses AS (
  SELECT
    be.brand_id,
    to_char((be.incurred_on)::timestamp with time zone, 'YYYY-MM'::text) AS yearmonth,
    SUM(COALESCE(be.amount, 0)) AS fixed_expenses
  FROM public.brand_expenses be
  GROUP BY be.brand_id, to_char((be.incurred_on)::timestamp with time zone, 'YYYY-MM'::text)
),
month_keys AS (
  SELECT brand_id, yearmonth FROM monthly_metrics
  UNION
  SELECT brand_id, yearmonth FROM monthly_expenses
)
SELECT
  mk.brand_id,
  mk.yearmonth,
  COALESCE(mm.gross_revenue, 0) AS gross_revenue,
  COALESCE(mm.discount_value, 0) AS discount_value,
  COALESCE(mm.net_revenue, 0) AS net_revenue,
  COALESCE(mm.commission_total, 0) AS commission_total,
  COALESCE(mm.cmv_total, 0) AS cmv_total,
  COALESCE(mm.gross_margin, 0) AS gross_margin,
  COALESCE(mm.adcost, 0) AS adcost,
  COALESCE(mm.contribution_after_media, 0) AS contribution_after_media,
  COALESCE(me.fixed_expenses, 0) AS fixed_expenses,
  COALESCE(mm.contribution_after_media, 0) - COALESCE(me.fixed_expenses, 0) AS operating_result,
  COALESCE(mm.items_sold, 0) AS items_sold,
  COALESCE(mm.qty_real, 0) AS qty_real,
  COALESCE(mm.order_count, 0) AS order_count,
  CASE
    WHEN COALESCE(mm.net_revenue, 0) > 0
      THEN COALESCE(mm.contribution_after_media, 0) / COALESCE(mm.net_revenue, 0)
    ELSE 0
  END AS contribution_margin_pct,
  CASE
    WHEN COALESCE(mm.net_revenue, 0) > 0
      THEN (COALESCE(mm.contribution_after_media, 0) - COALESCE(me.fixed_expenses, 0))
        / COALESCE(mm.net_revenue, 0)
    ELSE 0
  END AS operating_margin_pct
FROM month_keys mk
LEFT JOIN monthly_metrics mm
  ON mm.brand_id = mk.brand_id
 AND mm.yearmonth = mk.yearmonth
LEFT JOIN monthly_expenses me
  ON me.brand_id = mk.brand_id
 AND me.yearmonth = mk.yearmonth;
...
GRANT SELECT ON public.v_financial_monthly_canonical TO authenticated;
