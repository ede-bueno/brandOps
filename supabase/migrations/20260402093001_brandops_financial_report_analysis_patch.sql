...
BEGIN
  ...
  summary_payload AS (
    SELECT JSONB_BUILD_OBJECT(
      ...
      'breakEvenReason',
        CASE
          WHEN sm.fixed_expenses <= 0 THEN 'Sem despesas operacionais lançadas no período.'
          ...
          WHEN sm.contribution_margin_pct < 0.03 THEN 'Margem muito comprimida no recorte atual. O ponto de equilíbrio fica instável e não é exibido.'
          ELSE 'Valor mensal de RLD necessário para cobrir a média mensal de despesas fixas no recorte.'
        END,
